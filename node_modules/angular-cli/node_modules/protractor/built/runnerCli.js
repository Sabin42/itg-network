...
var logger = new logger2_1.Logger('runnerCli');
process.on('message', function (m) {
    switch (m.command) {
        case 'run':
            ...
            runner.on('testPass', function () { process.send({ event: 'testPass' }); });
            runner.on('testFail', function () { process.send({ event: 'testFail' }); });
            runner.on('testsDone', function (results) {
                process.send({ event: 'testsDone', results: results });
            });
            runner.run()
                .then(function (exitCode) { process.exit(exitCode); })
                .catch(function (err) {
